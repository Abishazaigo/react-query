...
> extends Subscribable<QueryObserverListener<TData, TError>> {
  ...
  setOptions(
    ...
    let optionalFetch = false
    let updateStaleTimeout = false
    let updateRefetchInterval = false
  ...
  private updateResult(): void {
    ...
    if (prevResult?.isSuccess === false && result.isSuccess === true) {
      notifyOptions.onSuccess = true
    } else if (prevResult?.isError === false && result.isError === true) {
  ...
  onQueryUpdate(_action: Action<TData, TError>): void {
    this.updateResult()
    if (this.hasListeners()) {
      this.updateTimers()
    }
